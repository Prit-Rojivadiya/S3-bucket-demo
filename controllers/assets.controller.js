...
module.exports = {
  ...
  async uploadFilesUsingMultipart(req, res) {
    try {
      const { files } = req;
      const result = await assetsService.uploadFilesUsingMultipart(files, req);
      return res.send(result);
    } catch (err) {
      console.log("could not upload file to s3 bucket", err);
    }
  ...
  async fetchLargeFile(req, res) {
    try {
      const { filePath } = req.body;
      return await assetsService.fetchLargeFile(filePath, res);
      // return res.send(result);
    } catch (err) {
      console.log("could not fetch file to s3 bucket", err);
    }
